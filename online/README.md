...
| 关闭房间 | 房主可删除房间，`rooms`/`moves`/`events`/`members` 四张表里该房间的数据一并清除，房间号随之释放；对手会看到"房间已不存在" |
...
| 关闭房间后重建同号房间，还会看到旧成员吗 | 不会。**关闭房间**会连同 `members` 表一起清空；另外**创建房间**成功时也会清一遍该房间号遗留的成员行，所以就算是用旧版本留下的数据，重建后也是空名单 |
| 为什么旧版本会出现"幽灵成员" | 早期版本的关闭房间只删了 `rooms`/`moves`/`events`，漏了 `members`，于是旧昵称与座位残留。现在两处都已修好 |
